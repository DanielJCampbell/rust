...
impl<'a> SpanUtils<'a> {
    ...
    pub fn filter_generated(&self, sub_span: Option<Span>, parent: Span) -> bool {
        if !generated_code(parent) {
            if sub_span.is_none() {
                // Non generated code is guaranteed to be Some - this is malformed.
                return true;
            }
            return false;
        }
        // Generated code occasionally has dud sub_spans - filter them.
        if sub_span.is_none() {
            return true;
        }
        // A generated span is deemed invalid if it is not a sub-span of the root
        // callsite. This rules out macro internal variables and most malformed spans.
